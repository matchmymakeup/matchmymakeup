...
const T = {
  ...
  zh: { appTagline:"扫描颜色，找到你的完美色号。", upload:"📷 上传", camera:"📸 相机", pickColor:"🎨 选色", uploadTapPrompt:"点击图片上的任意位置以提取颜色", choosePhoto:"选择照片", uploadFormats:"JPG · PNG · WEBP", changeImage:"↩ 更换图片", cameraPrompt:"将相机对准颜色并拍摄", startCamera:"开启相机", capture:"📍 拍摄", pickerPrompt:"使用调色盘或输入十六进制颜色代码", hexLabel:"色值", personalize:"✨ 个性化设置", skinToneLabel:"肤色", occasionLabel:"场合", categoryLabel:"类别", shopInLabel:"购物地区", allCountries:"🌍 全部", skinTones:{any:"不限",fair:"🤍 白皙",light:"🍑 浅色",medium:"🌼 中等",tan:"🌻 小麦色",deep:"🌑 深色"}, occasions:{any:"不限",daily:"☀️ 日常",office:"💼 职场",evening:"🌙 夜晚",wedding:"💍 婚礼",festival:"🎉 节日"}, findMatch:"💄 找到我的匹配", finding:"✨ 正在匹配...", pickFirst:"👆 请先选择颜色", scanning:"🔍 扫描产品数据库...", gettingAdvice:"✨ 获取美妆建议...", cameraError:"相机不可用。", library:"📚 我的收藏" },
...
export default function ColorScanner() {
  ...
  async function handleFindMatch() {
    ...
    try {
      ...
      const prompt = `You are a makeup expert named Maya. The user scanned color ${color.hex} (R:${color.r} G:${color.g} B:${color.b}). Skin tone: ${skinTone||'any'}. Occasion: ${occasion||'everyday'}. Country: ${country||'global'}.${categoryNote} Give 3 sentences of warm, personalized beauty advice for this color.`;
      ...
      // Save to scan history in library
      try {
        ...
        if (lib.scans.length > 50) lib.scans = lib.scans.slice(-50); // keep last 50
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"0 16px 48px"}}>
        ...
        <div style={{background:"white",borderRadius:20,padding:18,boxShadow:"0 4px 20px rgba(0,0,0,0.08)",marginBottom:14}}>
          ...
          <div style={{marginBottom:14}}>
            <label style={{display:"block",fontSize:11,color:"#888",fontWeight:600,marginBottom:8}}>{t.categoryLabel}</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {CATEGORIES.map(cat => (
                <button key={cat.id} onClick={()=>setCategory(cat.id==='all'?'':cat.id)}
                  style={{padding:"6px 12px",borderRadius:20,cursor:"pointer",border:(category===cat.id||(cat.id==='all'&&!category))?"2px solid #9d174d":"2px solid #e5e7eb",background:(category===cat.id||(cat.id==='all'&&!category))?"#fdf2f8":"white",color:(category===cat.id||(cat.id==='all'&&!category))?"#9d174d":"#555",fontSize:11,fontWeight:600}}>
                  {cat.emoji} {cat.label}
                </button>
              ))}
            </div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {langBtn("en","🇺🇸 EN")}{langBtn("hi","🇮🇳 HI")}{langBtn("pt","🇧🇷 PT")}{langBtn("zh","🇨🇳 ZH")}{langBtn("id","🇮🇩 ID")}{langBtn("ng","🇳🇬 NG")}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {countryBtn("",t.allCountries)}{countryBtn("USA","🇺🇸 USA")}{countryBtn("India","🇮🇳 India")}{countryBtn("Brazil","🇧🇷 Brazil")}{countryBtn("Indonesia","🇮🇩 Indonesia")}{countryBtn("Nigeria","🇳🇬 Nigeria")}{countryBtn("China","🇨🇳 China")}
            </div>
          </div>
        </div>
        ...
        <p style={{textAlign:"center",fontSize:11,color:"#bbb",marginTop:16}}>
          By using this app you agree to our <a href="/Terms" style={{color:"#9d174d"}}>Terms & Conditions</a> including data collection for market research.
        </p>
      </div>
    </div>
  );
