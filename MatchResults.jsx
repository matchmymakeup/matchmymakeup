...
export default function MatchResults() {
  ...
  function handleShopClick(product) {
    ...
    window.open(product.retailerUrl, '_blank', 'noopener,noreferrer');
  }
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#fdf2f8,#f3e8ff,#fce7f3)",fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <div style={{background:"white",padding:"16px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
        <div style={{maxWidth:560,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <button onClick={()=>window.location.href="/Library"} style={{background:"linear-gradient(135deg,#9d174d,#7c3aed)",color:"white",border:"none",borderRadius:10,padding:"8px 12px",cursor:"pointer",fontSize:11,fontWeight:700}}>
            {t.library}
          </button>
        </div>
      </div>
      ...
      <div style={{maxWidth:560,margin:"0 auto",padding:"20px 16px 60px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <div>
              ...
              <div style={{marginTop:8,display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                {record.category&&<span style={{background:"#fff7ed",color:"#c2410c",borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:600}}>{CATEGORY_EMOJI[record.category]} {record.category.replace('_',' ')}</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{background:"linear-gradient(135deg,#fdf2f8,#f3e8ff)",border:"1px solid #f9a8d4",borderRadius:20,padding:20,marginBottom:20,boxShadow:"0 4px 20px rgba(157,23,77,0.08)"}}>
          ...
          {adviceParagraphs.length > 0
            ? <div style={{fontSize:14,lineHeight:1.8,color:"#374151"}}>{adviceParagraphs.map((p,i)=><p key={i} style={{margin:"0 0 10px 0"}}>{p}</p>)}</div>
          ...
          <div style={{position:"relative",background:"linear-gradient(135deg,#fdf2f8,#f3e8ff)",border:"1px solid #f0abda",borderRadius:20,padding:"18px 44px 18px 18px",marginBottom:20}}>
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
              ...
              <div style={{flex:1}}>
                ...
                <a href="mailto:hello@matchmymakeup.app" style={{display:"inline-block",background:"linear-gradient(135deg,#9d174d,#7c3aed)",color:"white",borderRadius:12,padding:"9px 18px",fontSize:12,fontWeight:700,textDecoration:"none"}}>{t.upsellBtn}</a>
              </div>
            </div>
          </div>
        )}
        ...
        {products.length > 0 && (
          <div>
            <div style={{fontWeight:800,fontSize:16,color:"#1a1a1a",marginBottom:12}}>
              🎯 {products.length} {t.matchingProducts}
            </div>
              ...
              {products.map((p,i)=>(
                <div key={i} style={{background:"white",borderRadius:16,overflow:"hidden",boxShadow:"0 4px 16px rgba(0,0,0,0.08)",display:"flex",flexDirection:"column"}}>
                  <div style={{height:80,background:p.hexCode||"#f3e8ff",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
                    ...
                    {p.category&&(
                      <div style={{position:"absolute",top:6,right:6,background:"rgba(255,255,255,0.9)",borderRadius:8,padding:"2px 6px",fontSize:10,fontWeight:700,color:"#555"}}>
                  ...
                  <div style={{padding:"10px 10px 12px",flex:1,display:"flex",flexDirection:"column"}}>
                    ...
                    {p.price&&<div style={{fontSize:13,fontWeight:700,color:"#7c3aed",marginBottom:4}}>{p.currency}{p.price}</div>}
                    <div style={{fontSize:10,color:"#aaa",marginBottom:8}}>{t.colorDistance}: <b style={{color:"#374151"}}>{p.colorDistance}</b></div>
                    {p.retailerUrl&&(
                      <button
                        onClick={()=>handleShopClick(p)}
                        style={{display:"block",width:"100%",textAlign:"center",background:"linear-gradient(135deg,#9d174d,#7c3aed)",color:"white",borderRadius:10,padding:"7px 0",fontSize:11,fontWeight:700,border:"none",cursor:"pointer"}}>
                        {t.shopNow}
                      </button>
                    )}
                  </div>
        ...
        <p style={{textAlign:"center",fontSize:11,color:"#bbb",marginTop:24}}>
          <a href="/Terms" style={{color:"#9d174d"}}>Terms & Conditions</a> · Data collected anonymously for market research
        </p>
      </div>
    </div>
  );
